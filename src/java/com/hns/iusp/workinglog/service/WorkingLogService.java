/*
 * Copyright (C), 2013-2014, 南京华内斯信息技术有限公司
 * FileName: WorkingLogService.java
 * Author:   Tommy Xu
 * Date:     Nov 12, 2014 10:38:57 PM
 * Description: //模块目的、功能描述      
 * History: //修改记录
 * <author>      <time>      <version>    <desc>
 * 修改人姓名             修改时间            版本号                  描述
 */
package com.hns.iusp.workinglog.service;

import com.hns.iusp.common.bean.SessionUser;
import com.hns.iusp.utils.QueryResult;
import com.hns.iusp.workinglog.bean.WorkingLog;

/**
 * 〈一句话功能简述〉<br>
 * 〈功能详细描述〉
 * 
 * @author Tommy Xu
 * @see [相关类/方法]（可选）
 * @since [产品/模块版本] （可选）
 */
public interface WorkingLogService {

    int addWorkingLog(WorkingLog log);

    int saveDraft(WorkingLog log);
    int deleteDraftById(Integer id); //add by wangxuan 2014-11-23 删除草稿
    
    int updateWorkingLog(WorkingLog log);

    WorkingLog findWorkingLogById(Integer id);

    int findMyWorkingLogCount(WorkingLog log);

    QueryResult<WorkingLog> queryMyWorkingLogForPage(WorkingLog log, Integer pageNumber, Integer pageSize);

    int findWorkingLogCount(WorkingLog log, SessionUser user);

    QueryResult<WorkingLog> queryWorkingLogForPage(WorkingLog log, Integer pageNumber, Integer pageSize,
            SessionUser user);
    
    WorkingLog findWorkingLogByLogTime(String logTime, String userName);

    WorkingLog findWorkingLogByLogWeekTime(String year, String term, String week, String userName);
    
    int findMyWorkingLogCountBySpr(WorkingLog log);
}
