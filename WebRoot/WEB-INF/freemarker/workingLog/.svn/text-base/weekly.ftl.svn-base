...
<div style="width:1000px; margin:0px auto 0px auto; font-family:'微软雅黑';">
	...
	<div class="rightdiv" id="showPanel">
		...
		<div style="float:right; margin-top:19px; margin-right:50px; font-size:15px; color:#666;">
			<input type="hidden" id="year" value="${year}"/>
			<input type="hidden" id="term" value="${term}"/>
			<#if log.logStatus == '04'>
				${year}学年 第${term}学期 第${log.week}周
				<input type="hidden" id="week" value="${log.week}"/>
			<#else>
				${year}学年 第${term}学期
				第
				<select id="week">
					<#list 1..weekCount as week><option value="${week}" <#if log.week == week>selected='selected'</#if>>${week}</option></#list>
				<select>
				周
			</#if>
			<input type="hidden" id="logId" value="${log.id}"/>
			<input type="hidden" id="logStatus" value="${log.logStatus}"/>
			<input type="hidden" id="logType" value="02"/>
		</div>
		<table style="float:left; margin-top:20px;" border="1px" bordercolor="#D8D8D8">
			...
			<table  border="1px" bordercolor="#D8D8D8" class="ctab">
		    	<tr  style="background-color:#F5F5F5;height:30px;"><td >&nbsp;&nbsp;本周工作情况</td></tr>
		        <tr style="height:200px;">
		        	<td style="text-align:center">
		        		<textarea style="width:750px;height:195px;border:0;margin-top:5px" id="nowWorkInfo">${log.nowWorkInfo}</textarea>
		        	</td>
		        </tr>
		        <tr style="background-color:#F5F5F5;height:30px;"><td >&nbsp;&nbsp;下周工作计划</td></tr>
		        <tr style="height:200px;">
		        	<td style="text-align:center">
		        		<textarea style="width:750px;height:195px;border:0;margin-top:5px" id="nextWorkInfo">${log.nextWorkInfo}</textarea>
		        	</td>
		        </tr>
		    ...
		    <span  style=" float:right; margin-right:5px; margin-top:20px; ">
		    	<input type="button" id="weeklySubmit" value="&nbsp;&nbsp;提 &nbsp;交&nbsp;&nbsp;" style="border:0px; background-color:#63ACD9; width:100px; height:30px; color:#FFF;font-family:'微软雅黑'; font-size:16px;">&nbsp;&nbsp;
		        <input type="button" id="weeklySaveDraft" value="保存草稿" style=" background-color:#E3E3E3; width:100px; height:30px; border:0px;font-family:'微软雅黑';font-size:16px; color:#666" >&nbsp;&nbsp;
