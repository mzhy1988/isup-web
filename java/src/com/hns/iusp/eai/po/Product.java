package com.hns.iusp.eai.po;

public class Product {

	private double eaiproductid;
	private double adclientid;
	private double adorgid;
	private null created;
	private double createdby;
	private null updated;
	private double updatedby;
	private String name;
	private String value;
	private String medicinespec;
	private String description;
	private String isactive;
	private String isqastop;
	private String producttype;
	private String productstyle;
	private String manufacturer;
	private double guaranteedays;
	private double guaranteedaysmin;
	private double lbundleqty;
	private String uom;
	private String otctype;
	private String isforeign;
	private String certificateno;
	private String productcode;
	private String productusercode;
	private String productstatecode;
	private double versionstamp;
	private String productarea;
	private String stopreason;
	private double pricelist;
	private String categorytype;
	private double pricepo;
	private String productname;
	private String packagematerial;
	private double packageqty;
	private String upc;
	private double uomprecision;
	private String isstop;
	private String ishide;
	private double adserverid;
	private String productcontrollevel;
	private double pricesale;
	private String storagecondition;
	private String baseuom;
	private double mbundleqty;
	private double sbundleqty;
	private double version;
	private String invoiceprintrule;
	private double sourceserverid;
	private String englishname;
	private String pinyin;
	private String chemicalname;
	private String specificationno;
	private double eaiproductcategoryid;
	private String productstateid;
	private String certificatenoold;
	private String productspec;
	private String isonlinesale;
	private String effecttype;
	private String effectsubtype;
	private String isaudit;
	private String isindoubt;
	private double audituserid;
	private null audittime;
	private String isneedsync;
	private String medicinename;
	private String isdoubleconfirm;
	private String conservetype;
	private String issupervision;
	private String ishasdocument;
	private double multiplyrate;
	private double dividerate;
	private double masterproductid;
	private double masterserverid;
	private double taxrate;
	private String storageclass;
	private double potaxrate;
	private double mmanufacturerid;
	private String documentnote;
	private null certificatevaliddate;
	private String isbasicmedicine;
	private String productstructure;
	private String iscoldchain;
	private String businessscope;
	private String isotc;
	private String isinsurance;
	private String insurancetype;
	private String packagespec;

	public void setEaiproductid(double eaiproductid){
		this.eaiproductid=eaiproductid;
	}

	public double getEaiproductid(){
		return eaiproductid;
	}

	public void setAdclientid(double adclientid){
		this.adclientid=adclientid;
	}

	public double getAdclientid(){
		return adclientid;
	}

	public void setAdorgid(double adorgid){
		this.adorgid=adorgid;
	}

	public double getAdorgid(){
		return adorgid;
	}

	public void setCreated(null created){
		this.created=created;
	}

	public null getCreated(){
		return created;
	}

	public void setCreatedby(double createdby){
		this.createdby=createdby;
	}

	public double getCreatedby(){
		return createdby;
	}

	public void setUpdated(null updated){
		this.updated=updated;
	}

	public null getUpdated(){
		return updated;
	}

	public void setUpdatedby(double updatedby){
		this.updatedby=updatedby;
	}

	public double getUpdatedby(){
		return updatedby;
	}

	public void setName(String name){
		this.name=name;
	}

	public String getName(){
		return name;
	}

	public void setValue(String value){
		this.value=value;
	}

	public String getValue(){
		return value;
	}

	public void setMedicinespec(String medicinespec){
		this.medicinespec=medicinespec;
	}

	public String getMedicinespec(){
		return medicinespec;
	}

	public void setDescription(String description){
		this.description=description;
	}

	public String getDescription(){
		return description;
	}

	public void setIsactive(String isactive){
		this.isactive=isactive;
	}

	public String getIsactive(){
		return isactive;
	}

	public void setIsqastop(String isqastop){
		this.isqastop=isqastop;
	}

	public String getIsqastop(){
		return isqastop;
	}

	public void setProducttype(String producttype){
		this.producttype=producttype;
	}

	public String getProducttype(){
		return producttype;
	}

	public void setProductstyle(String productstyle){
		this.productstyle=productstyle;
	}

	public String getProductstyle(){
		return productstyle;
	}

	public void setManufacturer(String manufacturer){
		this.manufacturer=manufacturer;
	}

	public String getManufacturer(){
		return manufacturer;
	}

	public void setGuaranteedays(double guaranteedays){
		this.guaranteedays=guaranteedays;
	}

	public double getGuaranteedays(){
		return guaranteedays;
	}

	public void setGuaranteedaysmin(double guaranteedaysmin){
		this.guaranteedaysmin=guaranteedaysmin;
	}

	public double getGuaranteedaysmin(){
		return guaranteedaysmin;
	}

	public void setLbundleqty(double lbundleqty){
		this.lbundleqty=lbundleqty;
	}

	public double getLbundleqty(){
		return lbundleqty;
	}

	public void setUom(String uom){
		this.uom=uom;
	}

	public String getUom(){
		return uom;
	}

	public void setOtctype(String otctype){
		this.otctype=otctype;
	}

	public String getOtctype(){
		return otctype;
	}

	public void setIsforeign(String isforeign){
		this.isforeign=isforeign;
	}

	public String getIsforeign(){
		return isforeign;
	}

	public void setCertificateno(String certificateno){
		this.certificateno=certificateno;
	}

	public String getCertificateno(){
		return certificateno;
	}

	public void setProductcode(String productcode){
		this.productcode=productcode;
	}

	public String getProductcode(){
		return productcode;
	}

	public void setProductusercode(String productusercode){
		this.productusercode=productusercode;
	}

	public String getProductusercode(){
		return productusercode;
	}

	public void setProductstatecode(String productstatecode){
		this.productstatecode=productstatecode;
	}

	public String getProductstatecode(){
		return productstatecode;
	}

	public void setVersionstamp(double versionstamp){
		this.versionstamp=versionstamp;
	}

	public double getVersionstamp(){
		return versionstamp;
	}

	public void setProductarea(String productarea){
		this.productarea=productarea;
	}

	public String getProductarea(){
		return productarea;
	}

	public void setStopreason(String stopreason){
		this.stopreason=stopreason;
	}

	public String getStopreason(){
		return stopreason;
	}

	public void setPricelist(double pricelist){
		this.pricelist=pricelist;
	}

	public double getPricelist(){
		return pricelist;
	}

	public void setCategorytype(String categorytype){
		this.categorytype=categorytype;
	}

	public String getCategorytype(){
		return categorytype;
	}

	public void setPricepo(double pricepo){
		this.pricepo=pricepo;
	}

	public double getPricepo(){
		return pricepo;
	}

	public void setProductname(String productname){
		this.productname=productname;
	}

	public String getProductname(){
		return productname;
	}

	public void setPackagematerial(String packagematerial){
		this.packagematerial=packagematerial;
	}

	public String getPackagematerial(){
		return packagematerial;
	}

	public void setPackageqty(double packageqty){
		this.packageqty=packageqty;
	}

	public double getPackageqty(){
		return packageqty;
	}

	public void setUpc(String upc){
		this.upc=upc;
	}

	public String getUpc(){
		return upc;
	}

	public void setUomprecision(double uomprecision){
		this.uomprecision=uomprecision;
	}

	public double getUomprecision(){
		return uomprecision;
	}

	public void setIsstop(String isstop){
		this.isstop=isstop;
	}

	public String getIsstop(){
		return isstop;
	}

	public void setIshide(String ishide){
		this.ishide=ishide;
	}

	public String getIshide(){
		return ishide;
	}

	public void setAdserverid(double adserverid){
		this.adserverid=adserverid;
	}

	public double getAdserverid(){
		return adserverid;
	}

	public void setProductcontrollevel(String productcontrollevel){
		this.productcontrollevel=productcontrollevel;
	}

	public String getProductcontrollevel(){
		return productcontrollevel;
	}

	public void setPricesale(double pricesale){
		this.pricesale=pricesale;
	}

	public double getPricesale(){
		return pricesale;
	}

	public void setStoragecondition(String storagecondition){
		this.storagecondition=storagecondition;
	}

	public String getStoragecondition(){
		return storagecondition;
	}

	public void setBaseuom(String baseuom){
		this.baseuom=baseuom;
	}

	public String getBaseuom(){
		return baseuom;
	}

	public void setMbundleqty(double mbundleqty){
		this.mbundleqty=mbundleqty;
	}

	public double getMbundleqty(){
		return mbundleqty;
	}

	public void setSbundleqty(double sbundleqty){
		this.sbundleqty=sbundleqty;
	}

	public double getSbundleqty(){
		return sbundleqty;
	}

	public void setVersion(double version){
		this.version=version;
	}

	public double getVersion(){
		return version;
	}

	public void setInvoiceprintrule(String invoiceprintrule){
		this.invoiceprintrule=invoiceprintrule;
	}

	public String getInvoiceprintrule(){
		return invoiceprintrule;
	}

	public void setSourceserverid(double sourceserverid){
		this.sourceserverid=sourceserverid;
	}

	public double getSourceserverid(){
		return sourceserverid;
	}

	public void setEnglishname(String englishname){
		this.englishname=englishname;
	}

	public String getEnglishname(){
		return englishname;
	}

	public void setPinyin(String pinyin){
		this.pinyin=pinyin;
	}

	public String getPinyin(){
		return pinyin;
	}

	public void setChemicalname(String chemicalname){
		this.chemicalname=chemicalname;
	}

	public String getChemicalname(){
		return chemicalname;
	}

	public void setSpecificationno(String specificationno){
		this.specificationno=specificationno;
	}

	public String getSpecificationno(){
		return specificationno;
	}

	public void setEaiproductcategoryid(double eaiproductcategoryid){
		this.eaiproductcategoryid=eaiproductcategoryid;
	}

	public double getEaiproductcategoryid(){
		return eaiproductcategoryid;
	}

	public void setProductstateid(String productstateid){
		this.productstateid=productstateid;
	}

	public String getProductstateid(){
		return productstateid;
	}

	public void setCertificatenoold(String certificatenoold){
		this.certificatenoold=certificatenoold;
	}

	public String getCertificatenoold(){
		return certificatenoold;
	}

	public void setProductspec(String productspec){
		this.productspec=productspec;
	}

	public String getProductspec(){
		return productspec;
	}

	public void setIsonlinesale(String isonlinesale){
		this.isonlinesale=isonlinesale;
	}

	public String getIsonlinesale(){
		return isonlinesale;
	}

	public void setEffecttype(String effecttype){
		this.effecttype=effecttype;
	}

	public String getEffecttype(){
		return effecttype;
	}

	public void setEffectsubtype(String effectsubtype){
		this.effectsubtype=effectsubtype;
	}

	public String getEffectsubtype(){
		return effectsubtype;
	}

	public void setIsaudit(String isaudit){
		this.isaudit=isaudit;
	}

	public String getIsaudit(){
		return isaudit;
	}

	public void setIsindoubt(String isindoubt){
		this.isindoubt=isindoubt;
	}

	public String getIsindoubt(){
		return isindoubt;
	}

	public void setAudituserid(double audituserid){
		this.audituserid=audituserid;
	}

	public double getAudituserid(){
		return audituserid;
	}

	public void setAudittime(null audittime){
		this.audittime=audittime;
	}

	public null getAudittime(){
		return audittime;
	}

	public void setIsneedsync(String isneedsync){
		this.isneedsync=isneedsync;
	}

	public String getIsneedsync(){
		return isneedsync;
	}

	public void setMedicinename(String medicinename){
		this.medicinename=medicinename;
	}

	public String getMedicinename(){
		return medicinename;
	}

	public void setIsdoubleconfirm(String isdoubleconfirm){
		this.isdoubleconfirm=isdoubleconfirm;
	}

	public String getIsdoubleconfirm(){
		return isdoubleconfirm;
	}

	public void setConservetype(String conservetype){
		this.conservetype=conservetype;
	}

	public String getConservetype(){
		return conservetype;
	}

	public void setIssupervision(String issupervision){
		this.issupervision=issupervision;
	}

	public String getIssupervision(){
		return issupervision;
	}

	public void setIshasdocument(String ishasdocument){
		this.ishasdocument=ishasdocument;
	}

	public String getIshasdocument(){
		return ishasdocument;
	}

	public void setMultiplyrate(double multiplyrate){
		this.multiplyrate=multiplyrate;
	}

	public double getMultiplyrate(){
		return multiplyrate;
	}

	public void setDividerate(double dividerate){
		this.dividerate=dividerate;
	}

	public double getDividerate(){
		return dividerate;
	}

	public void setMasterproductid(double masterproductid){
		this.masterproductid=masterproductid;
	}

	public double getMasterproductid(){
		return masterproductid;
	}

	public void setMasterserverid(double masterserverid){
		this.masterserverid=masterserverid;
	}

	public double getMasterserverid(){
		return masterserverid;
	}

	public void setTaxrate(double taxrate){
		this.taxrate=taxrate;
	}

	public double getTaxrate(){
		return taxrate;
	}

	public void setStorageclass(String storageclass){
		this.storageclass=storageclass;
	}

	public String getStorageclass(){
		return storageclass;
	}

	public void setPotaxrate(double potaxrate){
		this.potaxrate=potaxrate;
	}

	public double getPotaxrate(){
		return potaxrate;
	}

	public void setMmanufacturerid(double mmanufacturerid){
		this.mmanufacturerid=mmanufacturerid;
	}

	public double getMmanufacturerid(){
		return mmanufacturerid;
	}

	public void setDocumentnote(String documentnote){
		this.documentnote=documentnote;
	}

	public String getDocumentnote(){
		return documentnote;
	}

	public void setCertificatevaliddate(null certificatevaliddate){
		this.certificatevaliddate=certificatevaliddate;
	}

	public null getCertificatevaliddate(){
		return certificatevaliddate;
	}

	public void setIsbasicmedicine(String isbasicmedicine){
		this.isbasicmedicine=isbasicmedicine;
	}

	public String getIsbasicmedicine(){
		return isbasicmedicine;
	}

	public void setProductstructure(String productstructure){
		this.productstructure=productstructure;
	}

	public String getProductstructure(){
		return productstructure;
	}

	public void setIscoldchain(String iscoldchain){
		this.iscoldchain=iscoldchain;
	}

	public String getIscoldchain(){
		return iscoldchain;
	}

	public void setBusinessscope(String businessscope){
		this.businessscope=businessscope;
	}

	public String getBusinessscope(){
		return businessscope;
	}

	public void setIsotc(String isotc){
		this.isotc=isotc;
	}

	public String getIsotc(){
		return isotc;
	}

	public void setIsinsurance(String isinsurance){
		this.isinsurance=isinsurance;
	}

	public String getIsinsurance(){
		return isinsurance;
	}

	public void setInsurancetype(String insurancetype){
		this.insurancetype=insurancetype;
	}

	public String getInsurancetype(){
		return insurancetype;
	}

	public void setPackagespec(String packagespec){
		this.packagespec=packagespec;
	}

	public String getPackagespec(){
		return packagespec;
	}

}
